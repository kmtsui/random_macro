...
void plot_bias_BnL(){
    ...
    for (int i=1;i<=h_alpha->GetNbinsX();i++){
        double timetofcut = timetof_min+(i-1)*timetof_dt;
        TFile f(Form("fitoutput_diffuser4_400nm_nominal_BnLPMT_%1.1f.root",timetofcut));
        TH1D* hist_alpha_result = (TH1D*)f.Get("hist_BnLPMT_angular_result");
        TH1D* hist_alpha_error_final = (TH1D*)f.Get("hist_BnLPMT_angular_error_final");
        ...
        h_alpha->SetBinContent(i,hist_alpha_result->GetBinContent(20));
        h_alpha->SetBinError(i,hist_alpha_error_final->GetBinContent(20));
...
return;
    ...
    TChain* fChain = new TChain("hitRate_pmtType0");
    ...
    TH1D* h_Timetof = new TH1D("","",1000,-955,-930);
