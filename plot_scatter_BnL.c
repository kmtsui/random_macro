void plot_scatter_BnL(){

//     gStyle->SetOptStat(0);
//     TFile* f400 = new TFile("diffuser4_400nm_scattering_map.root");
//     TFile* f350 = new TFile("diffuser4_350nm_scattering_map.root");
//     TH1D* h400 = (TH1D*)f400->Get("scattering_map_10_200");
//     TH1D* h350 = (TH1D*)f350->Get("scattering_map_10_200");
...
// return;
    ...
    fChain_digitized->Add("../production/out_diffuser4_400nm_nominal_*.root");
    ...
    TH1D* h_indirect = new TH1D("","",1000,-2,5);
    for (unsigned long int i=0;i<fChain_digitized->GetEntries();i++){
        ...
        // if (nRaySct>0 || (nReflec>0 && timetof>0.1))
        // {
        //     if (timetof_digi<15) h_timetof_10->Fill(PMT_id+0.5,nPE_digi);
        ...
        // }
        if (timetof>1 && timetof_digi<15) h_timetof_10->Fill(PMT_id+0.5,nPE_digi);
        else if (timetof_digi>15&&timetof_digi<200) h_timetof_10_200->Fill(PMT_id+0.5);
    }

    TFile* fout = new TFile("diffuser4_400nm_scattering_map_BnL.root","RECREATE");
    double sratio;
    ...
    gr->GetXaxis()->SetTitle("PE_{indirect,15<t_{D}<200}");
    gr->GetYaxis()->SetTitle("PE_{indirect,t_{D}<15}");
    gr->Draw("AP");
    ...
    h_timetof_10->Write("scattering_map_10_200");
