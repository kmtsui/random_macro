...
void plot_eff_var_joint()
{
    ...
    std::vector<TH1D*> h_full_10pc_vec = plot("joint_eff_study/10pc",0.015,0.005);
    ...
    TH1D* h_full_10pc_L = h_full_10pc_vec[0];
    TH1D* h_full_10pc_ang_mPMT = h_full_10pc_vec[1];
    TH1D* h_full_10pc_ang_BnL = h_full_10pc_vec[2];
    TH1D* h_full_2pc_L = h_full2pc_vec[0];
    TH1D* h_full_2pc_ang_mPMT = h_full2pc_vec[1];
    TH1D* h_full_2pc_ang_BnL = h_full2pc_vec[2];
    TCanvas* c1 = new TCanvas();
    c1->SetGridy();
    h_full_10pc_L->Draw("hist same");
    ...
    legend->AddEntry(h_full_10pc_L,"10 percent","l");
    legend->AddEntry(h_full_2pc_L,"2 percent","l");
    // legend->AddEntry(h_noout,"No outer","l");
    // legend->AddEntry(h_nomid,"No middle","l");
    legend->Draw("same");
    c1->SaveAs("Lalpha_eff_var_joint.pdf");

    h_full_10pc_ang_mPMT->Draw("hist");
    h_full_2pc_ang_mPMT->Draw("hist same");
    legend->Draw("same");
    c1->SaveAs("angular_mPMT_eff_var_joint.pdf");

    h_full_10pc_ang_BnL->Draw("hist");
    h_full_2pc_ang_BnL->Draw("hist same");
    legend->Draw("same");
    c1->SaveAs("angular_BnL_eff_var_joint.pdf");
return;
    int orders[] = {2,3};
