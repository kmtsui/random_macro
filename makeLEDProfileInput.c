void makeLEDProfileInput_1D()
{
    ...
    for (int i=0;i<nBins1;i++)
    {
        ...
        //power *= 1+gRandom->Gaus(0,0.05);
        //power = 1.;
        h_prior->SetBinContent(i+1,power);
    }
    h_prior->Write();
    TH1D* h_phi_prior = new TH1D("h_phi_prior","h_phi_prior",nBins1,0,nBins1);
    for (int i=0;i<nBins1;i++)
    {
        double power = (Variation_cosths[i]+Variation_cosths[i+1])/2./100.;
        //power = 0;
        h_phi_prior->SetBinContent(i+1,power);
    }
    h_phi_prior->Write();
    ...
    for(int r = 0; r < nBins1; ++r)
    {
        ...
        {
            cov_matrix[r][c] = r==c? pow(h_prior->GetBinContent(r+1)*0.01*1.e-3,2) : 0;
        }
    }
    cov_matrix.Write("cov_matrix_0percent");

    for(int r = 0; r < nBins1; ++r)
    {
        for(int c = 0; c < nBins1; ++c)
        {
            double power1 = (Variation_cosths[r]+Variation_cosths[r+1])/2./100.;
            double power2 = (Variation_cosths[c]+Variation_cosths[c+1])/2./100.;
            //cov_matrix[r][c] = r==c? power1*power2*0.01*0.01 : 0;
            cov_matrix[r][c] = r==c? pow(h_phi_prior->GetBinContent(r+1)*0.01*1.e-3,2) : 0;
        }
    }
    cov_matrix.Write("cov_matrix_phi_0percent");
    ...
    for (int i=1;i<=50;i++)
    {
        for(int r = 0; r < nBins1; ++r)
        {
            for(int c = 0; c < nBins1; ++c)
            {
                cov_matrix[r][c] = r==c? pow(h_prior->GetBinContent(r+1)*0.01*i,2) : 0;
            }
        }
        cov_matrix.Write(Form("cov_matrix_%ipercent",i));
        ...
        for(int r = 0; r < nBins1; ++r)
        {
            for(int c = 0; c < nBins1; ++c)
            {
                ...
                double power2 = (Variation_cosths[c]+Variation_cosths[c+1])/2./100.;
                //cov_matrix[r][c] = r==c? power1*power2*0.01*0.01 : 0;
                cov_matrix[r][c] = r==c? pow(h_phi_prior->GetBinContent(r+1)*0.01*i,2) : 0;
            }
        }
        cov_matrix.Write(Form("cov_matrix_phi_%ipercent",i));
    }
...
void makeLEDProfileInput_2D()
{
    ...
    TFile* fout = new TFile("led_prior_2D_bias5pc.root","RECREATE");
